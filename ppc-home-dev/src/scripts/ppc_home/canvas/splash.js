...
import { HOME_BG_PLANE_Z, positionEntries } from './grid';
...
function applySplashEntryPosition(entry, x, y, z, plane) {
	applySplashMeshPosition(entry.mesh, x, y, z, plane);
	entry.bgMesh.position.set(
		entry.mesh.position.x,
		entry.mesh.position.y,
		entry.mesh.position.z + HOME_BG_PLANE_Z,
	);
	entry.bgMesh.rotation.copy(entry.mesh.rotation);
}

function setSplashLayer(entry, renderOrder, depthWrite) {
	entry.mesh.renderOrder = renderOrder * 2 + 1;
	entry.mesh.material.depthWrite = depthWrite;
	entry.bgMesh.renderOrder = renderOrder * 2;
	entry.bgMesh.material.depthWrite = false;
}
...
function setPlaneMotionFrom(entry, target, plane) {
	...
	entry.bgMesh.scale.set(
		target.bgScale.x * scale,
		target.bgScale.y * scale,
		1,
	);
	if (plane.rotation.enabled) {
		const rotation = {
			x: resolveRotationFrom(plane.rotation.from.x, plane.rotation.rand),
			y: resolveRotationFrom(plane.rotation.from.y, plane.rotation.rand),
			z: resolveRotationFrom(plane.rotation.from.z, plane.rotation.rand),
		};
		entry.mesh.rotation.set(rotation.x, rotation.y, rotation.z);
		entry.bgMesh.rotation.set(rotation.x, rotation.y, rotation.z);
	} else {
		entry.mesh.rotation.set(0, 0, 0);
		entry.bgMesh.rotation.set(0, 0, 0);
	}
	entry.mesh.material.opacity = target.opacity * opacity;
	entry.bgMesh.material.opacity = target.opacity * opacity;
}
...
function addPlaneMotion(timeline, entry, target, plane, start, layout) {
	if (plane.scale.enabled) {
		...
		timeline.to(
			entry.bgMesh.scale,
			{
				x: target.bgScale.x * plane.scale.to,
				y: target.bgScale.y * plane.scale.to,
				duration: scaleDuration(layout.duration),
				ease: resolveGsapEase(layout.ease),
			},
			start,
	...
	const targets = new Map(
		visibleEntries.map((entry) => [
			entry,
			{
				...
				bgScale: entry.bgMesh.scale.clone(),
...
function resetSplashEntry(entry, target) {
	...
	entry.bgMesh.position.set(target.position.x, target.position.y, HOME_BG_PLANE_Z);
	entry.bgMesh.scale.set(target.bgScale.x, target.bgScale.y, 1);
	entry.bgMesh.rotation.set(0, 0, 0);
	entry.bgMesh.material.opacity = target.opacity;
	setSplashLayer(entry, 0, true);
}
...
function animateCardSplash(webgl, plane) {
	...
	dealOrder.forEach((entry, index) => {
		...
		setSplashLayer(entry, dealOrder.length - index, false);
	});
	webgl.render();
	webgl.splashTimeline = gsap.timeline({
		delay: layout.delay,
...
function animateSlideSplash(webgl, plane) {
	...
	slideOrder.forEach((entry, index) => {
		...
		setSplashLayer(entry, slideOrder.length - index, false);
	});
	webgl.render();
	webgl.splashTimeline = gsap.timeline({
		delay: layout.delay,
...
function animateTornadoSplash(webgl, plane) {
	...
	tornadoOrder.forEach((entry, index) => {
		...
		setSplashLayer(entry, tornadoOrder.length - index, false);
	});
	webgl.render();
	webgl.splashTimeline = gsap.timeline({
		delay: layout.delay,
