...
import { applyPaperCompile, clearPaperDeform } from './paper';
...
const HOME_BG_PLANE_COLOR = {
	white: 0xf3f3f3,
	black: 0x101010,
};

function getHomeBgPlaneColor() {
	return document.documentElement.getAttribute('data-bg') === 'white'
		? HOME_BG_PLANE_COLOR.white
		: HOME_BG_PLANE_COLOR.black;
}

export default class PpcHomeCanvas {
	constructor(container) {
		this.container = container;
		this.items = HOME_FEATURED_PROJECTS;
		...
		this.paperSegmentsX = 0;
		this.paperSegmentsY = 0;
		this.textures = [];
		this.entries = [];
		this.links = [];
		this.grid = null;
		...
		mobileQuery.addEventListener('change', this.resize);
		this.resize();
	}
	...
	createPlane(item, index, texture, cell, link) {
		const bgMaterial = new THREE.MeshBasicMaterial({
			color: getHomeBgPlaneColor(),
			transparent: true,
			depthWrite: false,
		});
		const bgMesh = new THREE.Mesh(this.geometry, bgMaterial);
		bgMesh.position.z = HOME_BG_PLANE_Z;
		...
		applyPaperCompile(material);
		const mesh = new THREE.Mesh(this.geometry, material);
		mesh.renderOrder = 1;
		this.scene.add(mesh);

		this.entries.push({ item, index, mesh, bgMesh, el: cell.el, link, cell });
	}

	syncMeshesToGrid() {
		this.entries.forEach(({ mesh, bgMesh }) => {
	...
	findItemIndexByUrl(url) {
		const path =
			new URL(url, document.baseURI).pathname.replace(/\/$/, '') || '/';

		return this.items.findIndex((item) => {
			const itemPath =
				new URL(item.href, document.baseURI).pathname.replace(/\/$/, '') ||
				'/';
	...
	killTransitionTweens() {
		...
		gsap.killTweensOf(this.entries.map(({ mesh }) => mesh.rotation));
		gsap.killTweensOf(this.entries.map(({ bgMesh }) => bgMesh.material));
		gsap.killTweensOf(this.entries.map(({ bgMesh }) => bgMesh.position));
		gsap.killTweensOf(this.entries.map(({ bgMesh }) => bgMesh.scale));
		gsap.killTweensOf(this.entries.map(({ bgMesh }) => bgMesh.rotation));
		clearPaperDeform(this);
	}
	...
	startLeaveToOther() {
		...
		const materials = this.entries.flatMap(({ mesh, bgMesh }) => [
			mesh.material,
			bgMesh.material,
		]);
	...
	startLeaveToSingle(entry) {
		...
		this.entries.forEach((item) => {
			otherMaterials.push(item.bgMesh.material);
			if (item !== entry) otherMaterials.push(item.mesh.material);
		});
		const fadeout =
	...
	completeTransition() {
		...
		if (this.transitionEntry) {
			...
			this.transitionEntry.bgMesh.visible = false;
			this.transitionEntry.bgMesh.position.z = HOME_BG_PLANE_Z;
		}
	...
	resize() {
		...
		if (this.grid && !this.transitioning && !this.splashing) {
			const next = isMobile() ? PPC_HOME_CANVAS_PARAMS.grid.mobile : PPC_HOME_CANVAS_PARAMS.grid.base;
			if (this.grid.grid.x !== next.x || this.grid.grid.y !== next.y) {
				createHomeGrid(this);
