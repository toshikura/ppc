...
export default class UsagiGridScroll {
	...
	getCloneSource(origins, sourceIndexes, cellIndex) {
		const x = Math.floor(cellIndex / this.grid.y);
		const y = cellIndex % this.grid.y;
		const neighborIndexes = [];

		if (y > 0) neighborIndexes.push(cellIndex - 1);
		if (y === this.grid.y - 1) neighborIndexes.push(x * this.grid.y);

		if (x > 0) {
			const previousX = x - 1;
			neighborIndexes.push(previousX * this.grid.y + y);
			neighborIndexes.push(previousX * this.grid.y + (y + this.grid.y - 1) % this.grid.y);
			neighborIndexes.push(previousX * this.grid.y + (y + 1) % this.grid.y);
		}

		if (x === this.grid.x - 1) {
			neighborIndexes.push(y);
			neighborIndexes.push((y + this.grid.y - 1) % this.grid.y);
			neighborIndexes.push((y + 1) % this.grid.y);
		}
	...
	initElements(){
		...
		const g = this.grid.x * this.grid.y - origins.length;
	...
	onWheel( e ){
		...
		if( e.target.closest('[data-usg-scroll-prevent]') ) return false;
		if( this.ignore && e.target.closest(this.ignore) ) return false;
		this.onResetCenterTween();
	...
	calcGrid() {
		...
		this.childWidth  = window.innerWidth / (this.grid.x - 1) * this.scale * this.autoScale;
		this.childHeight = window.innerWidth / (this.grid.x - 1) * this.aspectRatio * this.scale * this.autoScale;
		this.totalWidth  = this.childWidth * this.grid.x;
		this.totalHeight = this.childHeight * this.grid.y;
	...
	calcAutoScale(){
		const childWidth = window.innerWidth / (this.grid.x - 1) * this.scale;
		const childHeight = childWidth * (this.aspect.y / this.aspect.x);
		const totalWidth = childWidth * this.grid.x;
		const totalHeight = childHeight * this.grid.y;
	...
	destroy(){
		...
		this.wrapElem.querySelectorAll(`${this.blockClassName}.is-clone`).forEach((el) => {
			el.remove();
		});
		this.array = [];
		this.map = {};
	...
	set grid(value) {
		if (typeof value === 'object' && 'x' in value && 'y' in value) {
			...
			console.warn('Invalid grid value. It should be a number or an object with x and y properties.');
		}
	}
